...
//      }else{
//          str1 = '<li><a href="javascript:">'+(num[0]-6)+'</a></li>';
//      }
//      if (l > 1&&l < n+1) {
//          for (var i = 2; i < l + 1; i++) {
//              if(num[0]==i){
...
//      }
    ...
    var url=location.href;
    var reg=/\b\/\d{2}|\/\d{1}\b/;
    var num1=url.match(reg);
    var num=num1[0].replace("/","");
      ...
      if(num >= 1 && num <= 7){
          ...
          var y=num%7;//y[0,6]
              ...
              for(var x=0;x<=6;x++){
                  if(x==y){
                      str+='<li><a href="javascript:" class="'+active+'">' + num + '</a></li>';
                  }else{
                      if(num!=l){
                          var p=num-y+x;
                          str += '<li><a href="javascript:">' + p + '</a></li>';
                      }
      ...
      if(opts.hasNext){
          dataHtml += '<div class="next fr">'+'<a href="">'+ opts.next + '</a></div>';
      }
      if(opts.hasLastPage){
          dataHtml += '<div class="last fr">'+'<a href="">' + opts.lastPage + '</a></div>';
      }
          dataHtml += '<ul class="pagingUl">'+ str + '</ul>';
      if(opts.hasFirstPage){
          dataHtml += '<div class="first fr">'+'<a href="">'+ opts.firstPage + '</a></div>';
      }
      if(opts.hasPrv){
          dataHtml += '<div class="prv fr">'+'<a href="">' + opts.prv + '</a></div>';
      }
...
//        }
        opts.callBack(pageshow+1);
           pageshow2 = pageshow+1;
          this.children[0].href="/pj/oceanEngin/"+pageshow2;
      ...
      obj.on('click', '.prv', function () {
        ...
        opts.callBack(pageshow-1);
          pageshow=pageshow-1
          this.children[0].href="/pj/oceanEngin/"+pageshow;
      });
      ...
      obj.on('click', '.first', function(){
        ...
        opts.callBack(1);
        fpagePrv(0);
        this.children[0].href="/pj/oceanEngin/"+1;
...
//        }//当前最后一页
        ...
        this.children[0].href="/pj/oceanEngin/"+99;
      ...
      obj.on('click', 'li', function(){
        ...
        function fpageShow(activePage){
          ...
          var str1 = '';
          for(i=0;i<n;i++){
            str1 += '<li><a href="/pj/oceanEngin/2" class="">' + (pageStart+i) + '</a></li>'
          }
